...
class sidb_surface<Lyt, false> : public Lyt
{
  public:
     ...
     */
    void assign_sidb_defect(const typename Lyt::cell& c, const sidb_defect& d) noexcept
    {
     ...
     */
    [[nodiscard]] sidb_defect get_sidb_defect(const typename Lyt::cell& c) const noexcept
    {
     ...
     */
    [[nodiscard]] std::set<coordinate<Lyt>> affected_sidbs(const typename Lyt::cell& c) const noexcept
    {
        ...
        if (const auto d = get_sidb_defect(c); d.type != sidb_defect_type::NONE)
        {
                 ...
                 ++y)
            {
                ...
                {
                    if (const auto affected = typename Lyt::cell{x, y, c.z}; Lyt::is_within_bounds(affected))
                    {
