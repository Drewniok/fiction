...
class charge_distribution_surface<Lyt, false> : public Lyt
{
  public:
    ...
    struct charge_distribution_storage
    {
      ...
      public:
        ...
        std::pair<uint64_t, int8_t> defect_history_gray_code{};
        /**
        ...
        using defect_distance_matrix = std::vector<std::vector<double>>;
        /**
         * The potential matrix is a vector of vectors storing the electrostatic potentials.
         */
        using defect_potential_matrix = std::vector<std::vector<double>>;

        uint64_t defect_charge_index{};

        uint64_t max_defect_charge_index{};

        std::vector<typename Lyt::cell> defect_cell_order{};

        std::vector<sidb_defect> defect_order{};
    ...
    void generate_defect_order()
    {
        for (const auto& defect : strg->defects)
        {
            strg->defect_cell_order.emplace_back(defect.first);
            strg->defect_order.emplace_back(defect.second);
        }
    }
    /**
     * This function assigns a defect to the layout.
    ...
    void update_local_potential_defect_change() noexcept
    {
        if (strg->defect_history_gray_code.first != -1)
        {
            const auto cell_charge =
                static_cast<double>(strg->defect_charge_index[strg->defect_history_gray_code.first]);
            const auto charge_diff = (cell_charge - strg->defect_history_gray_code.second);
            for (uint64_t j = 0u; j < strg->sidb_order.size(); j++)
            {
                strg->local_pot[j] +=
                    strg->defect_potential_matrix[strg->defect_history_gray_code.first][j] * charge_diff;
            }
        }
    }
    //        else
    ...
    void update_after_defect_charge_change(const bool dependent_cell_fixed = true, const bool energy_calculation = true,
                                           const bool& consider_history = false) noexcept
    {
        this->update_local_potential_defect_change(consider_history);
    ...
    void defect_index_to_charge_distribution(uint64_t new_gray_code, uint64_t old_gray_code) noexcept
    {
        strg->defect_history_gray_code = {};
        ...
        if (diff != 0)
        {
            ...
            {
                strg->defect_history_gray_code.first  = index_changed;
                strg->defect_history_gray_code.second = sign_old;
                this->assign_defect_charge_state(index_changed, sign_new);
            }
            else
            {
                strg->defect_history_gray_code.first  = index_changed + 1;
                strg->defect_history_gray_code.second = sign_old;
                this->assign_defect_charge_state(index_changed + 1, sign_new);
            }
        }
        else
        {
            strg->defect_history_gray_code.first  = -1;
            strg->defect_history_gray_code.second = 0;
        }
    }
    ...
    void set_defect_charge_index_by_gray(const uint64_t gray_code, const uint64_t gray_code_old,
                                         const bool  dependent_cell_fixed    = true,
                                         const bool& recompute_system_energy = true,
                                         const bool& consider_history        = false) noexcept
    {
        if (gray_code <= strg->max_defect_charge_index)
        {
            this->assign_defect_index_by_gray_code(gray_code, gray_code_old);
            this->update_after_defect_charge_change(dependent_cell_fixed, recompute_system_energy, consider_history);
        }
    }
    ...
    void assign_defect_index_by_gray_code(uint64_t gray_code, uint64_t gray_code_old) noexcept
    {
        strg->def.first = gray_code;
        this->defect_index_to_charge_distribution(gray_code, gray_code_old);
    }
    /**
    ...
    void init_defects()
    {
        strg->initialize_defect_distance_matrix();
        strg->initialize_defect_potential_matrix();
        strg->max_defect_charge_index = std::pow(2, strg->defect_order.size()) - 1;
    }
  ...
  private:
    ...
    void initialize_defect_distance_matrix() const noexcept
    {
        strg->defect_distance_matrix =
            std::vector<std::vector<double>>(strg->defect_order.size(), std::vector<double>(this->num_cells(), 0));

        for (uint64_t i = 0u; i < strg->defect_order.size(); ++i)
        {
            for (uint64_t j = 0u; j < strg->sidb_order.size(); j++)
            {
                strg->defect_distance_matrix[i][j] =
                    sidb_nanometer_distance<Lyt>(*this, strg->defect_order[i], strg->sidb_order[j], strg->phys_params);
            }
        }
    }

    void initialize_defect_potential_matrix() const noexcept
    {
        strg->defect_potential_matrix =
            std::vector<std::vector<double>>(strg->defect_order.size(), std::vector<double>(this->num_cells(), 0));
        ...
        for (uint64_t i = 0u; i < strg->defect_order.size(); ++i)
        {
            for (uint64_t j = 0u; j < strg->sidb_order.size(); j++)
            {
                strg->defect_potential_matrix[i][j] = chargeless_potential_defect_with_given_distance(
                    strg->defect_distance_matrix[i][j], strg->defect_order[i]);
            }
        }
    }
};
