...
{
    auto lyt = read_sqd_layout<sidb_100_cell_clk_lyt_siqad>("/Users/jandrewniok/CLionProjects/fiction_fork/experiments/"
    ...
    uint64_t counter = 0;
        for (const auto& gate : all_gate_candidates)
        {
            ...
            if (counter <5)
            {
                write_sqd_layout(gate, fmt::format("/Users/jandrewniok/Desktop/large_efficient_gate_{}.sqd", counter));
            }
        }
...
//    for (const auto& gate : all_gate_candidates)
//    {
//        if (is_operational(gate, std::vector<tt>{create_and_tt()}, is_operational_params{params.simulation_parameters})
//                .first == operational_status::OPERATIONAL)
//        {
//            counter++;
//            write_sqd_layout(gate, fmt::format("/Users/jandrewniok/Desktop/cx_efficient_gate_{}.sqd", counter));
//        }
//    }
